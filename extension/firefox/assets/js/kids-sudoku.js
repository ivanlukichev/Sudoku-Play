(function () {
  ...
  function buildCleanAbsoluteUrl(pathname, params) {
    ...
    return origin + buildCleanRelativeUrl(normalizePathname(pathname), params);
  }
  ...
  function syncCleanLocation(pathname, seed, includeSeed) {
    var cleanUrl = buildCleanRelativeUrl(getHistoryPathname(pathname), includeSeed ? { p: seed } : {});
    try {
  ...
  function KidsSudokuApp(root) {
    ...
    this.path = normalizePathname(window.location.pathname || "/");
  ...
  function setupDailyKidsBlock() {
    ...
    document.querySelectorAll("[data-kids-daily-link]").forEach(function (node) {
      if (EXTENSION_PAGE && typeof EXTENSION_PAGE.getLocalUrl === "function") {
        node.setAttribute("href", EXTENSION_PAGE.getLocalUrl("/mini-sudoku/", { p: seed }));
      } else {
